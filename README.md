...
The compiler, Planner, and restricted Host executor also support the first complete asynchronous Host tracer: one managed RabbitMQ Queue, one gated systemd Worker, one generation-specific systemd Task, and one stable Schedule. An approved Plan installs immutable Worker and Task generations, verifies the Worker while intake is closed, opens intake, installs a digest-pinned nonresident Schedule runtime, and hands a stable systemd timer to the exact Task generation. Each due occurrence is committed to a SQLite ledger before Task delivery; the Task publishes with confirmation and the Worker manually acknowledges the same stable message identity. A Worker-only replacement keeps its candidate gated while fencing and draining or releasing the previous consumer, then verifies active message processing before committing authority and retaining the previous generation. A failed active candidate is fenced and the exact previous Worker is restored and verified against the same stable message identity. Controller interruption at every handoff boundary resumes under a higher fence from fresh Queue, systemd, message, and durable authority evidence; completed mutations are not replayed, ambiguous state pauses, and stale results remain non-authoritative. Schedule handoff between revisions, general retry/dead-letter behavior, overlap, missed-run catch-up, retention expiry, Queue replacement, and Host/data-loss recovery remain separate work. See [Asynchronous Host execution](docs/asynchronous-plan-preview.md) and the [Worker recovery evidence](docs/evidence/2026-09-26-worker-handoff-recovery.md).
