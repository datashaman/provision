...
Gimme's Laravel focus is not a Provision requirement. Transfer its verified operational lessons, not its framework-specific defaults, commands, or file layout. Provision's core must work for non-Laravel applications; any Laravel convenience belongs in optional application-level integration.
...
| Laravel worker and scheduler process management | Adapt behavioral evidence only | Keep tested ownership and handoff behavior while representing Worker, Task, Queue, and Schedule as independent logical roles. No `artisan`, Horizon, or Laravel scheduler assumption enters the core contract. |
...
1. **Transfer acceptance evidence, not operational ownership.** Convert Gimme's host bootstrap, plan-staleness, secret-redaction, runtime-pin, and rollout failure cases into Provision-facing specifications. Create a non-Laravel application fixture that exercises the core HTTP, Worker, Task, and Schedule paths, then expand it to other first-class roles. Do not point Provision at a Gimme-managed Caddy site, service unit, or store yet.
