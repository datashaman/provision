...
The deployment principal may invoke only the root-owned host executor entrypoint. The implemented local and SSH paths share one Operation Handler contract: acquire a renewable fenced execution lease for the Environment mutation, journal intent, and sign an Ed25519 authorization with a maximum-five-minute lifetime. This is separate from the optional team-member Environment Lease in the product model. The proof binds the exact approved and current Plan, Application, Environment, Host Target, operator, observed executor digest, typed operation digest, attempt, and fencing token. The executor verifies the root-owned public key and bootstrap record and rejects replays and stale fencing tokens durably. It permits the dependency-free `stageArtifact` operation into a fixed digest-addressed cache, followed only by dependency-gated typed operations that install an immutable native Generation, create and start its exact hardened systemd candidate unit on loopback, and re-observe that unit and Generation before verifying liveness, readiness, and revision-bound candidate checks. Only a candidate with a successful exact verification in the host's own root-owned authorization history may enter the typed Endpoint switch. That operation rechecks the candidate and planned previous Generation, atomically replaces only the Plan-owned Caddy server, verifies the stable route, retains the previous unit and Generation directory, and durably records active and previous identities. A separate signed operation then evaluates the Health Contract through the stable Endpoint. Failure permits rollback only after the exact retained previous Generation passes direct health checks; the executor restores its Caddy route, verifies it again through the stable Endpoint, and durably swaps the active and previous identities. After successful stable verification, a further signed operation proves the exact active and previous identities and stable route, waits the full declared ordinary-HTTP handoff bound, stops only that previous unit, and retains its unit definition and immutable Generation directory. A final signed rollback-window operation then proves the exact successful drain and all restart material, derives the deadline from the trusted switch timestamp plus the Environment's declared rollback window, and records the previous Generation as restartable without deleting or otherwise mutating that material. Expiry and cleanup remain a separate policy-rooted operation. Any unproved recovery is recorded as uncertain and blocks dependent work. The executor accepts no arbitrary shell command, caller-selected filesystem path, or caller-shaped route mutation. Failed candidate verification removes only the failed candidate after refusing cleanup of the recorded active Generation; a rejected Caddy load leaves the previous route active. The Operation Handler observes before apply or resumption, verifies kind-specific structured results, and exposes the operation's declared recovery mode. `deployment resume` selects the latest interrupted or uncertain operation from the durable journal, atomically binds the new intent to the superseded attempt, acquires a higher fence, and re-observes the target before deciding whether the result is already satisfied, the typed operation remains pending, or state is uncertain. It can reconcile a Caddy route loaded before its active-generation record, a rollback completed before its result was committed, a completed bounded drain whose response was lost, and a durable rollback-window marker whose response was lost; state matching neither signed Generation pauses with explicit evidence and recovery instructions. The State Backend conditionally commits an authoritative outcome and releases the execution lease atomically. A stale result cannot append to that journal; its submitted observation is retained in a separate non-authoritative rejection log.
