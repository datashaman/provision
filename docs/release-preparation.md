...
The native HTTP Host Deployment lifecycle is now support-qualified through both direct-local and remote-SSH execution: candidate preparation, Endpoint switch, stable verification, bounded ordinary-HTTP drain, and exact previous-Generation rollback-window recording use the same engine seams and produce equivalent host outcomes. Expiry does not authorize deletion: cleanup, quarantine, and garbage collection remain separate unimplemented policy decisions. WebSocket and other long-lived-stream drain guarantees also remain unsupported rather than silently inferred from ordinary HTTP behavior.
